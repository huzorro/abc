	...
	private void configInit() {
		...
		for(Object service : list) {
			try {
				...
				logger.error("start client to init services fails", e.getCause());
	...
	public List<String> duplexstreamService(List<String> configList) {
		...
		for(ClientServices service : services) {
			try {
				...
				logger.error("duplex stream globalVars Initialize [FAILS]", e.getCause());
				continue;
			} 
			try {
				...
				logger.error("duplex stream service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream pool watch service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream submit service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream reserve deliver service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream deliver msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream response msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("duplex stream receive msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
	...
	public List<String> upstreamService(List<String> configList) {
		...
		for(ClientServices service : services) {
			try {
				...
				logger.error("up stream globalVars Initialize [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream pool watch service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream receive msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
		}
		return resultList;
	...
	public List<String> downstreamService(List<String> configList) {
		...
		for(ClientServices service : services) {
			try {
				...
				logger.error("down stream globalVars Initialize [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream pool watch service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream deliver service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream reserve deliver service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream response msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("down stream deliver msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
		}		
		return resultList;
