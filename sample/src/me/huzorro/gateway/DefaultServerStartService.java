	...
	private void configInit() {
		...
		for(Object service : list) {
			try {
				...
				logger.error("start server to init services fails", e.getCause());
	...
	public List<String> upstreamService(List<String> configList) {
		...
		for(ServerServices service : services) {
			...
			try {
				...
				resultList.add("up stream deliver service to init [OK]");
				logger.error("up stream deliver service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream reserve deliver service to init [FAILS]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream deliver msg plugin manager service to init [OK]", e.getCause());
				continue;
			}
			try {
				...
				logger.error("up stream response msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
		}
		return resultList;
	...
	public List<String> downstreamService(List<String> configList) {
		...
		for (ServerServices service : services) {
			...
			try {
				...
				logger.error("down stream receive msg plugin manager service to init [FAILS]", e.getCause());
				continue;
			}
		}
		return resultList;
